jQuery( document ).ready( function( $ ) {
	...
	default_text( '#wp-content-editor-container textarea', '<a href="%%instagram-image-url%%" title="%%caption-no-tags-escaped%%" target="_blank"><img alt="%%caption-no-tags-escaped%%" src="%%image%%" class="%%instagram-media-type%% %%image-class%%"></a>' );

	// Image saving
	if ( $( 'input[name="_instagrate_pro_settings[post_save_media]"]' ).is( ':checked' ) ) {
		$( '.image-saving' ).show();
	}
	...
	// Users Images
	if ( $( 'select[name="_instagrate_pro_settings[instagram_images]"] option:selected' ).val() === 'users' ) {
		$( '.instagram_user' ).show();
	}

	$( 'select[name="_instagrate_pro_settings[instagram_images]"]' ).change( function() {
		if ( $( 'option:selected', this ).val() === 'users' ) {
			$( '.instagram_user' ).show();
		} else {
			$( '.instagram_user' ).hide();
		}
	...
	// Location Images
	if ( $( 'select[name="_instagrate_pro_settings[instagram_images]"] option:selected' ).val() === 'location' ) {
		$( '.instagram_location' ).show();
	}

	$( 'select[name="_instagrate_pro_settings[instagram_images]"]' ).change( function() {
		if ( $( 'option:selected', this ).val() === 'location' ) {
			$( '.instagram_location' ).show();
		} else {
			$( '.instagram_location' ).hide();
		}
	} );

	// Schedule
	if ( $( 'select[name="_instagrate_pro_settings[posting_frequency]"] option:selected' ).val() === 'schedule' ) {
		$( '.schedule' ).show();
	}

	$( 'select[name="_instagrate_pro_settings[posting_frequency]"]' ).change( function() {
		if ( $( 'option:selected', this ).val() === 'schedule' ) {
			$( '.schedule' ).show();
		} else {
			$( '.schedule' ).hide();
		}
	...
	// Manual
	if ( $( 'select[name="_instagrate_pro_settings[posting_frequency]"] option:selected' ).val() === 'manual' ) {
		$( '.manual' ).show();
	}
	...
	if ( $( 'select[name="_instagrate_pro_settings[posting_multiple]"] option:selected' ).val() === 'single' ) {
		$( '.single_post' ).show();
	}

	// Multiple Images - Grouped
	if ( $( 'select[name="_instagrate_pro_settings[posting_multiple]"] option:selected' ).val() === 'group' ) {
		$( '.grouped' ).show();
	}
	...
	$( 'select[name="_instagrate_pro_settings[posting_multiple]"]' ).change( function() {
		...
		if ( $( 'option:selected', this ).val() === 'single' ) {
			$( '.single_post' ).show();
		}
	...
	$( 'select[name="_instagrate_pro_settings[post_type]"]' ).change( function() {
		var post_type = $( 'option:selected', this ).val();
		...
		var taxonomy = $( 'select[name="_instagrate_pro_settings[post_taxonomy]"] option:selected' ).val();
		// Reload Terms
	...
	$( 'select[name="_instagrate_pro_settings[post_taxonomy]"]' ).change( function() {
		var taxonomy = $( 'option:selected', this ).val();
		change_taxonomy_terms( taxonomy );
		return false;
	} );
	...
	$( '#igp-logout' ).on( 'click', function( e )  {
		e.preventDefault();
	...
	$( '#igp-refresh' ).on( 'click', function( e ) {
		e.preventDefault();
		document.getElementById( "igp-refresh" ).disabled = true;
		$( '.igp-avatar' ).addClass('processing');
		var post_id = $( '#post_ID' ).val();
		$.post( ajaxurl,
			{
				action: 'igp_refresh',
			...
			function( data ) {
				...
				$( '.igp-avatar' ).removeClass('processing');
	...
	function image_stats( stats ) {
		...
		$( 'label.moderate .stat' ).text( '0' );
		$.each( stats, function( key, val ) {
			$( 'label.' + key + ' .stat' ).text( val.Total );
			if ( 'moderate' === key ) {
				if ( val.Total > 0 ) {
					$( 'label.' + key + ' .stat' ).parents( '.igp-status' ).removeClass( 'hide' );
				} else {
					$( 'label.' + key + ' .stat' ).parents( '.igp-status' ).addClass( 'hide' );
				}
			}
		} );
	}
	...
	$( '#igp-load-images' ).on( 'click', function() {
		...
		$.post( ajaxurl,
			...
			function( data ) {
				var img_status = get_image_status();
				$.each( data.images, function( key, val ) {
					$( '#igp-images' ).append( '<li><a class="edit-image ' + val.media_type + '" rel="' + val.id + '" href="#"><img class="' + ((data.load) ? img_status : val.status) + '" width="70" alt="" src="' + val.images.thumbnail.url + '"><span class="video-ind"></span></a><input id="' + val.id + '" class="igp-bulk" type="checkbox"></li>' );
				} );
	...
	function set_status( status ) {
		...
		var images = $( '#igp-images input:checkbox:checked' ).map( function() {
			return this.id;
		} ).get().join( "," );
		if ( images === '' ) {
	...
	$( 'input[name="_instagrate_pro_settings[moderate_images]"]' ).change( function() {
		$( '#igp-images' ).addClass( 'processing' );
		$( '#igp-set-bulk-status' ).attr( 'disabled', 'disabled' );

		var from_status = 'moderate';
		var to_status = 'pending';
		if ( $( this ).is( ':checked' ) ) {
			from_status = 'pending';
			to_status = 'moderate';
		}
		...
		$.post( ajaxurl,
			...
			function( data ) {
				image_stats( data.stats );
				$( '#igp-images img.' + from_status ).addClass( to_status );
				$( '#igp-images img' ).removeClass( from_status );
				$( '#igp-images' ).removeClass( 'processing' );
				alert( data.message );
			}, 'json' );
		return false;
	...
	// Change Username
	function change_username( element ) {
		// Get Users Id
	...
	$( 'input[name="_instagrate_pro_settings[instagram_user]"]' ).change( function() {
		change_username( this );
	} );

	$( 'input[name="_instagrate_pro_settings[instagram_user]"]' ).keypress( function( e ) {
		if ( e.which === 13 ) {
			e.preventDefault();
			change_username( this );
			return false;
	...
	$( 'input[name="_instagrate_pro_settings[instagram_location]"]' ).keypress( function( e ) {
		if ( e.which === 13 ) {
			e.preventDefault();
			change_location( this );
		}
	...
	$( 'input[name="_instagrate_pro_settings[instagram_hashtags]"]' ).change( function() {
		change_hashtags( this );
	} );

	$( 'input[name="_instagrate_pro_settings[instagram_hashtags]"]' ).keypress( function( e ) {
		if ( e.which === 13 ) {
			e.preventDefault();
			change_hashtags( this );
			return false;
	...
	function change_stream() {
		...
		$.post( ajaxurl,
			...
			function( data ) {
				$( '#igp-images' ).empty();
				var i = 0;
				var img_class = get_image_status();
				$.each( data.images, function( key, val ) {
					i++;
					$( '#igp-images' ).append( '<li><a class="edit-image ' + val.type + '" rel="' + val.id + '" href="#"><img class="' + img_class + '" width="70" alt="" src="' + val.images.thumbnail.url + '"><span class="video-ind"></span></a><input id="' + val.id + '" class="igp-bulk" type="checkbox"></li>' );
				} );
	...
	function get_image_status() {
		var img_class = 'pending';
		if ( $( 'input[name="_instagrate_pro_settings[moderate_images]"]' ).is( ':checked' ) ) {
			img_class = 'moderate';
		}
			...
			$.ajax( {
				...
				success: function( response ) {
					if ( response.error ) {
						...
						if ( response.license_status === 'activated' ) {
