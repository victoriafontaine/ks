...
;(function($) {
   ...
   'use strict'
	...
	var teamCarousel = function(){
		if ( $().owlCarousel ) {
			$(".roll-team:not(.roll-team.no-carousel), .panel-grid-cell .latest-news-wrapper").not('.roll-team.type-b').owlCarousel({
				navigation : false,
				pagination: true,
				responsive: true,
				items: 3,
	...
	var headerClone = function() { 
		...
		if ( $('.header-contact').length ) {
			...
			$('.admin-bar.no-hero .site-header, .admin-bar.no-hero .site-header.float-header').css('top', contactHeight + 32 );
		}
