...
function sydney_widgets_init() {
	...
	if ( get_theme_mod('activate_bh_widgets') ) {
		register_sidebar( array(
			'name'          => __( 'Below header area', 'sydney' ),
			'id'            => 'sidebar-header',
			'description'   => '',
			'before_widget' => '<aside id="%1$s" class="widget %2$s">',
			'after_widget'  => '</aside>',
	...
	if ( defined( 'SITEORIGIN_PANELS_VERSION' ) ) {
		...
		register_widget( 'Sydney_Employees_Type_B' );
		register_widget( 'Sydney_Latest_News' );
		register_widget( 'Sydney_Latest_News_Type_B' );
		register_widget( 'Sydney_Timeline' );
		register_widget( 'Sydney_FP_Contact_Info' );
		register_widget( 'Sydney_Contact_Info' );
		register_widget( 'Sydney_Portfolio' );
		register_widget( 'Sydney_Image_widget' );
		register_widget( 'Sydney_Pricing_Tables' );
	}
...
if ( defined( 'SITEORIGIN_PANELS_VERSION' ) ) {
	...
	require get_template_directory() . "/widgets/fp-employees-type-b.php";
	require get_template_directory() . "/widgets/fp-latest-news.php";
	require get_template_directory() . "/widgets/fp-latest-news-type-b.php";
	require get_template_directory() . "/widgets/fp-timeline.php";
	require get_template_directory() . "/widgets/fp-contact.php";
	require get_template_directory() . "/widgets/fp-portfolio.php";
	require get_template_directory() . "/widgets/fp-image.php";
	require get_template_directory() . "/widgets/fp-pricing.php";
...
function sydney_scripts() {
	...
	wp_enqueue_style( 'sydney-style', get_stylesheet_uri(), '', '20170502' );
	...
	wp_enqueue_script( 'sydney-main', get_template_directory_uri() . '/js/main.min.js', array('jquery'), '20170502', true );

	if (is_page_template('page-templates/page_contact.php')) {
		wp_enqueue_script( 'sydney-gmap', get_template_directory_uri() . '/js/gmap3.min.js', array('jquery'), '', true );
	}
...
function sydney_contact_info() {
	$toggle_contact = get_theme_mod('toggle_contact');
	$phone 	 		= get_theme_mod('contact_phone', '99.123.456');
	$email 	 		= get_theme_mod('contact_email', 'office@site.com');
	$address 		= get_theme_mod('contact_address', '12 Street, New York City');
	if ($toggle_contact) {
		echo '<div class="header-contact"><div class="container">';
			if ($phone) {
			echo '<span class="header-phone">';
				echo '<i class="fa fa-phone"></i>' . $phone;
			echo '</span>';
			}
			if ($email) {
			echo '<span class="header-email">';
				echo '<i class="fa fa-envelope"></i>' . $email;
			echo '</span>';
			}
			if ($address) {
			echo '<span class="header-address">';
				echo '<i class="fa fa-home"></i>' . $address;
			echo '</span>';
			}
		echo '</div></div>';					
	}
...
require get_template_directory() . '/inc/customizer-wc.php';
...
/**
 * Category metabox for page templates
 */
require get_template_directory() . '/inc/pt-metabox.php';

/**
 * Contact metabox
 */
require get_template_directory() . '/inc/contact-metabox.php';
 ...
 */
function sydney_pro_updater() {
    require( get_template_directory() . '/updater/theme-updater.php' );
}
add_action( 'after_setup_theme', 'sydney_pro_updater' );

/**
 * License notice
 */
function sydney_pro_admin_notice(){

	$license = get_option( 'sydney-pro-ii_license_key' );

	if (!$license) {
	    echo '<div class="updated">';
	    echo 	'<p>Please activate your license key for Sydney Pro to get the latest theme updates automatically. You can get your key from your <a href="http://athemes.com/your-account/" target="_blank">aThemes account</a> and activate it <a href="' . menu_page_url( 'sydney-pro-ii-license' ,0 ) . '">here</a></p>';
	    echo '</div>';
	}
}
add_action('admin_notices', 'sydney_pro_admin_notice');